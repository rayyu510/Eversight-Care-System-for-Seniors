// src/surveillance-center/services/cameraService.ts
import {
    CameraDevice,           // Changed from Camera
    CameraConfiguration,    // Changed from CameraSettings  
    CameraPreset,          // For camera control
    CameraGroup,           // Keep same
    CameraStatus,
    CameraStream,
    CameraCapabilities,
    CameraMaintenance
...
// Define control interface for camera operations
...
class CameraService {
    ...
    async getCameras(): Promise<CameraDevice[]> {
        // Mock camera data using new CameraDevice interface
        return [
            {
                id: 'CAM_12345678',
                name: 'Main Entrance',
    ...
    async getCameraMetrics(cameraId: string, timeRange: '1h' | '24h' | '7d' = '24h'): Promise<CameraMetrics> {
        ...
        // Mock metrics based on camera status
        return {
            uptime: camera.status.uptime || 0,
            bandwidth: camera.stream.bitrate / 1000, // Convert to MB/s
            storageUsed: Math.random() * 50, // GB
            recordingHours: 720, // Last 30 days
    ...
    subscribe(event: string, callback: Function): () => void {
        ...
        // Mock subscription - implement real WebSocket/SSE connection
        return () => {
